...
export const getDailyLesson = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: profile } = await db.from("profiles").select("name, school_grade, curriculum_type, xp, level, streak_days").eq("id", userId).maybeSingle();
    ...
    return {
      ...
      weakSkills,
      plan,
      recommendation: hasMastery ? `Сегодня лучше сфокусироваться на теме «${focusSkill.title}».` : "Сначала пройди короткую диагностику — она нужна, чтобы AI не учил вслепую.",
    };
  });
...
export const recordExerciseAttempt = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const mistakeType = classifyMistakeFromAnswer({ prompt: data.prompt, studentAnswer: data.studentAnswer, expectedAnswer: data.expectedAnswer });
    const isCorrect = mistakeType === "correct";

    const { data: attempt, error } = await db.from("exercise_attempts").insert({
      user_id: userId,
      skill_id: skill.id,
    ...
    const { data: current } = await db.from("student_skill_mastery").select("mastery_score, attempts_count, correct_attempts").eq("user_id", userId).eq("skill_id", skill.id).maybeSingle();
